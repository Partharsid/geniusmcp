...
/**
 * RFC v2 composite score formula:
 *   qualityComposite = presetWeights * {durability, vitality, importability, codeQuality}
 *   overall = 0.35 * queryFit + 0.50 * qualityComposite + 0.15 * evidenceConfidence
 *
 * This prevents repo popularity from dominating (queryFit is first-class)
 * and penalizes half-verified candidates (evidenceConfidence matters).
 */
export function compositeScore(breakdown: ScoreBreakdown, preset: RankingPreset): number {
  ...
  return (
    0.35 * breakdown.queryFit +
    0.50 * qualityComposite +
